...
$sql="insert into equips(name,bank,number,branch,code,iban,email,amount,current,comment) values
('$name','$bank','$number','$branch','$code','$iban','$email','$amount','$current','$comment')";
if($conn->query($sql))
{
    $sql = "SELECT * FROM equips WHERE email='$email'";
    $result = mysqli_query($conn, $sql);
                        ...
                        <table style="width:30px;">
                            <tr>
                                ...
                                <td style="width:150px;height:30px;border:2px solid green;background-color:green;">
                                    <a href="transfers.php?email=<?=$email?>&current=<?=$current?>"
